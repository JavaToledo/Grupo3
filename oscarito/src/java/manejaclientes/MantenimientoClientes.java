...
/** * * @author Darka */
public class MantenimientoClientes extends HttpServlet {
    /**
...
//        synchronized (ManejadorDb.class) {
            try {
                listadoClientes = ManejadorDb.buscarCliente(dni);
            } catch (Exception ex) {
                System.out.println(ex.getMessage());
                tablaClientes="<h3> No se pudo conectar a la base de datos</h3>";
            }
//        }
            ...
            if (listadoClientes.size()>0){
                ...
                for (Cliente c : listadoClientes) {
                    tablaClientes+="<tr>"
                            ...
                            + "<td>"+ c.getTelefono()+ "</td>"
                            + "<td>"+ c.getEmail()+ "</td>"
                            + "<td> <a href='actualizarcliente?dni="
                                    +c.getDni()+"'>Modificar </a>"
                            + "<td> <a href='borrarcliente?dni="
                                    +c.getDni()+"'>Borrar </a>"
                            + "</tr>";
                }
                tablaClientes+="</table>";
            }else {
                tablaClientes="<h3> No se encontraron datos de clientes </h3>";
            }
        }else{
                tablaClientes="<h3> No se pudo conectar a la base de datos</h3>";
        }
